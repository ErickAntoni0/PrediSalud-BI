"undefined" != typeof jQuery && function(c) {
    ...
    function l(e, t, i) {
        if (1 !== arguments.length)
            return 3 === arguments.length ? new Date(0, 0, 0, e, t, i) : 2 === arguments.length ? new Date(0, 0, 0, e, t, 0) : new Date(0, 0, 0);
        var n = e;
    ...
    c.TimePicker.prototype = {
        ...
        _items: function(e, t) {
            ...
            for (-1 === e.options.timeFormat.indexOf("m") && e.options.interval % 60 != 0 && (e.options.interval = 60 * Math.max(Math.round(e.options.interval / 60), 1)), i = t ? l(t) : e.options.startTime ? l(e.options.startTime) : l(e.options.startHour, e.options.startMinutes), n = new Date(i.getTime() + 864e5); i < n;)
                this._isValidTime(e, i) && (a = c("<li>").addClass("ui-menu-item").appendTo(r), c("<a>").addClass("ui-corner-all").text(c.fn.timepicker.formatTime(e.options.timeFormat, i)).appendTo(a), a.data("time-value", i)),
                i = new Date(i.getTime() + 60 * e.options.interval * 1e3);
            return r.children()
        },
        ...
        _move: function(e, t, i) {
            var n,
                r = this;
            r.closed() && r.open(e),
            r.active && (n = r.active[t + "All"](".ui-menu-item").eq(0)).length ? r.activate(e, n) : r.activate(e, r.viewport.children(i))
        },
        register: function(e, t) {
            var i = this,
                n = {};
            n.element = c(e),
            n.element.data("TimePicker") || (n.options = c.metadata ? c.extend({}, t, n.element.metadata()) : c.extend({}, t), n.widget = i, c.extend(n, {
                next: function() {
                    return i.next(n)
                },
                previous: function() {
        ...
        select: function(e, t) {
            var i = this,
                n = !1 === e ? i.instance : e;
            i.setTime(n, c.fn.timepicker.parseTime(t.children("a").text())),
            i.close(n, !0)
        },
        ...
        setTime: function(e, t, i) {
            var n = e.selectedTime;
            if ("string" == typeof t && (t = e.parse(t)), t && t.getMinutes && this._isValidTime(e, t)) {
                if (t = l(t), e.selectedTime = t, e.element.val(e.format(t, e.options.timeFormat)), i)
                    return e
            } else
                e.selectedTime = null;
        ...
        option: function(t, e, i) {
            ...
            c.each(n, function(e) {
                t.options[e] = n[e],
                t.rebuild = t.rebuild || -1 < c.inArray(e, r)
            }),
    ...
    c.fn.timepicker.parseTime = (d = (m = [[/^(\d+)$/, "$1"], [/^:(\d)$/, "$10"], [/^:(\d+)/, "$1"], [/^(\d):([7-9])$/, "0$10$2"], [/^(\d):(\d\d)$/, "$1$2"], [/^(\d):(\d{1,})$/, "0$1$20"], [/^(\d\d):([7-9])$/, "$10$2"], [/^(\d\d):(\d)$/, "$1$20"], [/^(\d\d):(\d*)$/, "$1$2"], [/^(\d{3,}):(\d)$/, "$10$2"], [/^(\d{3,}):(\d{2,})/, "$1$2"], [/^(\d):(\d):(\d)$/, "0$10$20$3"], [/^(\d{1,2}):(\d):(\d\d)/, "$10$2$3"]]).length, function(e) {
        var t,
            ...
            a = !1,
            s = !1;
        if (void 0 === e || !e.toLowerCase)
            return null;
        e = e.toLowerCase(),
        i = !(t = /a/.test(e)) && /p/.test(e),
