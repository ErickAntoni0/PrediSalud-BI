...
mapbox && (dataset = {}, onlyValues = (series = [["AUS", 50], ["FRA", 77], ["SWZ", 58], ["USA", 26], ["RUS", 46]]).map(function(e) {
    return e[1]
    ...
    geographyConfig: {
        ...
        popupTemplate: function(e, o) {
            if (o)
                return ['<div class="hoverinfo">', "<strong>", e.properties.name, "</strong>", "<br>Count: <strong>", o.numberOfThings, "</strong>", "</div>"].join("")
        }
    }
}), zoomOpts = {
    scaleFactor: 1,
...
mapEuropeBox && (mapEurope = new Datamap({
    ...
    geographyConfig: {
        ...
        popupTemplate: function(e, o) {
            return ['<div class="px-2 py-1 bg-white">', '<small class="text-muted">' + e.properties.NAME, ": <strong>" + o.numberOfThings, "</strong></small></div>"].join("")
        }
    },
    scope: "europe",
    responsive: !0,
    ...
    setProjection: function(e, o) {
        var r = d3.geo.mercator().center([25, 50]).rotate([0, 0]).scale(180).translate([e.offsetWidth / 2, e.offsetHeight / 2]);
        return {
            path: d3.geo.path().projection(r),
            projection: r
        }
    }
}), window.addEventListener("resize", function() {
